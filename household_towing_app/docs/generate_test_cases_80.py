...
def extract_steps_list(flow_text):
    if not flow_text:
        return []
...
def generate_csv(text, output_file):
    ...
    for uc in use_cases:
        ...
        # 2. Main Flow - This is one sequence of steps
        ...
        if main_flow_match:
            ...
            steps = extract_steps_list(main_flow)
        ...
        # 3. Alternative flows - EACH STEP IS A SEPARATE SCENARIO (TEST CASE)
        ...
        if alt_flow_match:
            ...
            alt_steps = extract_steps_list(alt_flow)
            
            for i, step_text in enumerate(alt_steps):
                # Generate a short title from the text
                short_title = (step_text[:40] + '...') if len(step_text) > 40 else step_text
                cases.append({
                    "id": case_id,
                    "title": f"Alternative Flow {i+1}: {short_title}",
                    "description": goal,
                    "preconditions": precondition,
                    "postconditions": "",
                    "tags": "actor:" + actor.replace(' ', '_'),
        ...
        # 4. Exception flows - EACH STEP IS A SEPARATE SCENARIO (TEST CASE)
        exc_flow_match = re.search(r'(Exception:)(.*?)(User Action:|$)', flows_text, re.DOTALL)
        if exc_flow_match:
            exc_flow = exc_flow_match.group(2).strip()
            exc_steps = extract_steps_list(exc_flow)
            
            for i, step_text in enumerate(exc_steps):
                short_title = (step_text[:40] + '...') if len(step_text) > 40 else step_text
                cases.append({
                    "id": case_id,
                    "title": f"Exception Flow {i+1}: {short_title}",
                    "description": goal,
                    "preconditions": precondition,
                    "postconditions": "",
                    "tags": "actor:" + actor.replace(' ', '_'),
...
generate_csv(text, r'c:\code\DUALSERVE\household_towing_app\docs\Qase_Test_Cases_Detailed.csv')
print(f"Generated {r'c:\code\DUALSERVE\household_towing_app\docs\Qase_Test_Cases_Detailed.csv'}")
